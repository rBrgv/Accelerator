...
import { generateMetadataReportMarkdown } from "@/server/reports/metadataReport";
...
async function generateReport(scanOutput: ScanOutput, requestId: string) {
  ...
  // Generate markdown
  const markdown = generateMetadataReportMarkdown(scanOutput);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logger.error({ error: error.message, stack: error.stack }, "Failed to generate metadata report");
    return NextResponse.json(
